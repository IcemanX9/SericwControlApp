 ...
 */
	...
	/**
	 * Authenticates an android client by checking that the correct token has been received and has not expired
	 *
	 * @return boolean
	 */
	public function apiIsAuthorised($userId, $token) {
		$this->id = $userId;
		//check if the token is correct
		if ($token == $this->field("apiToken")) {
			//check if the token is still active (1 hour inactivity allowed)
			if (time() - $this->field("apiTokenTime") > 3600) {
				//not authorised and reset the token which has expired
				$this->saveField("apiToken", "");
				$this->saveField("apiTokenTime", 0);
				return false;
			}
			else {
				//authorised, renew token life
				$this->saveField("apiTokenTime", time());
				return true;
			}
		}
		//otherwise tokens don't match, not authorised
		else return false;
	}
